...
-- Requer função public.handle_updated_at (script 003_create_triggers.sql).
...
CREATE TABLE IF NOT EXISTS public.comanda_produtos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  comanda_id UUID NOT NULL REFERENCES public.comandas(id) ON DELETE CASCADE,
  produto_estoque_id UUID NOT NULL REFERENCES public.estoque_produtos(id) ON DELETE RESTRICT,
  nome TEXT NOT NULL,
  preco_unitario DECIMAL(10, 2) NOT NULL CHECK (preco_unitario >= 0),
  quantidade INTEGER NOT NULL DEFAULT 1 CHECK (quantidade >= 1),
  created_at TIMESTAMPTZ DEFAULT NOW()
